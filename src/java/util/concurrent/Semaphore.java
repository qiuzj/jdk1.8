...
public class Semaphore implements java.io.Serializable {
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        final int nonfairTryAcquireShared(int acquires) {
            for (;;) {
                int available = getState(); // 可用
                int remaining = available - acquires; // 剩余
                if (remaining < 0 || // 剩余信号量许可数<0，则获取失败
                    compareAndSetState(available, remaining)) // 仅当信号量许可数小于0，或者获取成功时才返回
                    return remaining;
            }
        }
    ...
    static final class FairSync extends Sync {
        ...
        protected int tryAcquireShared(int acquires) {
            for (;;) {
                if (hasQueuedPredecessors()) // 与非公平锁区别
                    return -1; // 非头节点则获取失败
                int available = getState(); // 可用信号量许可数
                int remaining = available - acquires; // 剩余信号量许可数
                if (remaining < 0 || // 剩余信号量许可数<0，则获取失败
                    compareAndSetState(available, remaining)) // 仅当信号量许可数小于0，或者获取成功时才返回
                    return remaining;
            }
